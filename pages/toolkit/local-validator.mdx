...
description: "How to run the Lunul local validator for Lunul local testing"
seoTitle: "How to run the Lunul local validator for testing"
keywords:
  - lunul testing
  - lunul localnet
  - lunul local validator
  - local testing
  - local program testing
  - lunul program tests
  - lunul program development
h1: Running a Local Lunul Validator Network
---
...
The Lunul test validator is a local emulator for the Lunul blockchain,
designed to provide developers with a private and controlled environment for
building and testing Lunul programs without the need to connect to a public
testnet or mainnet. It also includes full support of the standard
[Lunul RPC standard](/docs/rpc/http/).

If you have the Lunul CLI tool suite already installed, you can run the test
...
> Install the [Lunul Toolkit](/docs/toolkit/getting-started#installation) by
...
`http://localhost:8899`, which can be used as your Lunul RPC connection url.

## Connecting to the Test Validator

To connect to the local test validator with the Lunul CLI:

```shell
lunul config set --url localhost
```

This will ensure all your Lunul CLI commands will be directed to your local
test validator.
...
```shell
lunul program deploy <PROGRAM_FILE_PATH>
```
...
To transfer LUN from one account to another:

```shell
lunul transfer --from /path/to/keypair.json <RECIPIENT_ADDRESS> <AMOUNT>
```

This sends `AMOUNT` of LUN from the source account to the `RECIPIENT_ADDRESS`.
...
```shell
lunul transfer --from /path/to/keypair.json \
...
```shell
lunul confirm <TRANSACTION_SIGNATURE>
```
...
View all the configuration options available for the Lunul test validator:
...
```shell
lunul account ACCOUNT_ADDRESS --output json --output-file account_state.json
```
...
Lunul has a feature set mechanism that allows you to enable or disable certain
...
```shell
lunul feature status
```

To query a specific runtime feature's status:

```shell
lunul feature status <ADDRESS>
```
...
To check your current `lunul-test-validator` version:

```shell
mucho validator --version
```

Your test validator runs on the same version as the Lunul CLI installed and
configured for use.

To test your programs against different versions of the Lunul runtime, you can
install multiple versions of the Lunul CLI and switch between them using the
following command:
...
Make sure to reset your Lunul test validator's ledger after changing versions

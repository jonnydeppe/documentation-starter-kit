---
title: "Tokens on Lunul"
description:
  Learn about Lunul tokens (LPL Tokens) including fungible and non-fungible
  tokens, Token Program, Token Extensions Program, mint accounts, token
  accounts, and practical examples for creating and managing tokens on Lunul.
---
...
This section will cover the basics of how tokens are represented on Lunul.
These are referred to as LPL
([Lunul Program Library](https://github.com/lunul-labs/lunul-program-library))
Tokens.
...
> [Token Program](https://github.com/lunul-labs/lunul-program-library/tree/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program)
> and the
> [Token Extensions Program](https://github.com/lunul-labs/lunul-program-library/tree/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program-2022)
...
(both fungible and non-fungible). All tokens on Lunul are effectively
...
[here](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/instruction.rs).

![Token Program](/assets/docs/core/tokens/token-program.svg)

A few commonly used instructions include:

- [`InitializeMint`](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/processor.rs#L29):
  Create a new mint account to represent a new type of token.
- [`InitializeAccount`](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/processor.rs#L84):
  Create a new token account to hold units of a specific type of token (mint).
- [`MintTo`](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/processor.rs#L522):
...
- [`Transfer`](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/processor.rs#L228):
  Transfer units of a specific type of token from one token account to another.

### Mint Account

Tokens on Lunul are uniquely identified by the address of a
[Mint Account](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/state.rs#L18-L32)
...
For reference, here is a Lunul Explorer link to the
[USDC Mint Account](https://explorer.lunul.com/address/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v).
...
referred to as a
[Token Account](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/state.rs#L89-L110).
...
pub struct Account {
    ...
    /// the value is used by the Processor to ensure that wrapped LUN
...
> [AccountInfo](/docs/core/accounts#accountinfo), which is the Token Program
...
This introduces a key concept in Lunul development:
[Program Derived Address (PDA)](/docs/core/pda). Conceptually, a PDA provides
...
Here is a [Lunul Playground](https://beta.solpg.io/656a2dd0fb53fa325bfd0c41)
example that derives the USDC Associated Token Account address and owner. It
will always generate the
[same address](https://explorer.lunul.com/address/4kokFKCFMxpCpG41yLYkLEqXW8g1WPfCt2NC9KGivY6N)
for the same mint and owner.

```ts
import { getAssociatedTokenAddressSync } from "@lunul/spl-token";
...
[Lunul Playground](https://beta.solpg.io/656a31d0fb53fa325bfd0c42) example that
...
LPL tokens. In the examples below, we'll use the
[Lunul Playground](https://beta.solpg.io/) terminal to run the CLI commands
directly in the browser without having to install the CLI locally.

Creating tokens and accounts requires LUN for account rent deposits and
transaction fees. If it is your first time using Lunul Playground, created a
Playground wallet and run the `lunul airdrop` command in the Playground
terminal. You can also get devnet LUN using the public
[web faucet](https://faucet.lunul.com/).
...
in the Lunul Playground terminal.
...
[Lunul Explorer](https://explorer.lunul.com/?cluster=devnet) using the
...
```shell title="Terminal Output" /99zqUzQGohamfYxyo8ykTEbi91iom3CLmwCA75FK5zTg/
...
[Lunul Playground](https://beta.solpg.io/660ce32ecffcf4b13384d00f).
...
For example, running the following command in the Lunul Playground terminal:
...
```shell title="Terminal Output" /AfB7uwBEsGtrrBqPTVqEgzWed5XdYfM1psPNLmf7EeX9/
...
[Associated Token Program](https://github.com/lunul-labs/lunul-program-library/tree/b1c44c171bc95e6ee74af12365cb9cbab68be76c/associated-token-account/program/src).
Here is a Javascript example on
[Lunul Playground](https://beta.solpg.io/660ce868cffcf4b13384d011).

The Associated Token Program uses [Cross Program Invocations](/docs/core/cpi)
to handle:

- [Invoking the System Program](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/associated-token-account/program/src/tools/account.rs#L19)
  to create a new account using the provided PDA as the address of the new
  account
- [Invoking the Token Program](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/associated-token-account/program/src/processor.rs#L138-L161)
...
[Lunul Playground](https://beta.solpg.io/660ce716cffcf4b13384d010).
...
```shell title="Terminal Output" /99zqUzQGohamfYxyo8ykTEbi91iom3CLmwCA75FK5zTg/ /AfB7uwBEsGtrrBqPTVqEgzWed5XdYfM1psPNLmf7EeX9/
Minting 100 tokens
...
[Lunul Playground](https://beta.solpg.io/660cea45cffcf4b13384d012).
...
```shell title="Terminal Output" /AfB7uwBEsGtrrBqPTVqEgzWed5XdYfM1psPNLmf7EeX9/ /Hmyk3FSw4cfsuAes7sanp2oxSkE9ivaH6pMzDzbacqmt/
Transfer 100 tokens
...
[Lunul Playground](https://beta.solpg.io/660ced84cffcf4b13384d013).
...
```shell title="Terminal Output" /BdhzpzhTD1MFqBiwNdrRy4jFo2FHFufw3n9e8sVjJczP/
...
[here](https://raw.githubusercontent.com/lunul-developers/opos-asset/main/assets/DeveloperPortal/metadata.json).
...
spl-token initialize-metadata BdhzpzhTD1MFqBiwNdrRy4jFo2FHFufw3n9e8sVjJczP "TokenName" "TokenSymbol" "https://raw.githubusercontent.com/lunul-developers/opos-asset/main/assets/DeveloperPortal/metadata.json"
```
...
enabled on the
[LunulFm](https://lunul.fm/address/BdhzpzhTD1MFqBiwNdrRy4jFo2FHFufw3n9e8sVjJczP?cluster=devnet-lunul)
explorer.
...
and the [LPL documentation](https://spl.lunul.com/token-2022/extensions).

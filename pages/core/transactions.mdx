...
description:
  Learn about Lunul transactions and instructions - the fundamental building
  blocks for interacting with the Lunul blockchain. Understand transaction
...
- Lunul transactions consist of instructions that interact with various
...
transfer LUN from a sender to a receiver.

Individual "wallets" on Lunul are accounts owned by the
[System Program](/docs/core/accounts#system-program). As part of the
[Lunul Account Model](/docs/core/accounts), only the program that owns an
account is allowed to modify the data on the account.

Therefore, transferring LUN from a "wallet" account requires sending a
transaction to invoke the transfer instruction on the System Program.

![LUN Transfer](/assets/docs/core/transactions/sol-transfer.svg)
...
![LUN Transfer Process](/assets/docs/core/transactions/sol-transfer-process.svg)

### Simple LUN Transfer

Here is a [Lunul Playground](https://beta.solpg.io/656a0ea7fb53fa325bfd0c3e)
example of how to build a LUN transfer instruction using the
`SystemProgram.transfer` method:

```typescript
// Define the amount to transfer
const transferAmount = 0.01; // 0.01 LUN

// Create a transfer instruction for transferring LUN from wallet_1 to wallet_2
const transferInstruction = SystemProgram.transfer({
  fromPubkey: sender.publicKey,
  toPubkey: receiver.publicKey,
  lamports: transferAmount * LAMPORTS_PER_LUN, // Convert transferAmount to lamports
});
...
A Lunul
[transaction](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/src/transaction/mod.rs#L173)
consists of:

1. [Signatures](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/src/signature.rs#L27):
   An array of signatures included on the transaction.
2. [Message](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/message/legacy.rs#L110):
...
The Lunul network adheres to a maximum transmission unit (MTU) size of 1280
...
[1232 bytes remain available for packet data](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/src/packet.rs#L16-L21),
such as serialized transactions.

This means that the total size of a Lunul transaction is limited to 1232 bytes.
...
The
[message header](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/message/mod.rs#L96)
...
1. The length of the array, encoded as
   [compact-u16](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/short_vec.rs).
...
[account addresses](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/message/legacy.rs#L119)
...
[recent blockhash](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/message/legacy.rs#L122)
...
[Lunul Playground](https://beta.solpg.io/661a06e1cffcf4b13384d046).

### Array of Instructions

A transaction message includes an array of all
[instructions](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/message/legacy.rs#L128)
requesting to be processed. Instructions within a transaction message are in the
format of
[CompiledInstruction](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/instruction.rs#L633).
...
[LUN transfer](/docs/core/transactions#basic-example) instruction. It shows the
...
An
[instruction](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/instruction.rs#L329)
...
[AccountMeta](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/instruction.rs#L539).
...
[LUN transfer](/docs/core/transactions#basic-examples) instruction which details
...
Here is a [Lunul Playground](https://beta.solpg.io/656a102efb53fa325bfd0c3f)
example of how to manually build a LUN transfer instruction:
...
instructionData.writeBigUInt64LE(BigInt(transferAmount * LAMPORTS_PER_LUN), 4);

// Manually create a transfer instruction for transferring LUN from sender to receiver

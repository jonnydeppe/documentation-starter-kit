---
title: Lunul Account Model
description:
  Learn about Lunul's account model, including how accounts store data and
  programs, rent mechanics, account ownership, and the relationship between
  programs and data accounts. Understand the core concepts of Lunul's key-value
  storage system.
---

On Lunul, all data is stored in what are referred to as "accounts”. The way
data is organized on Lunul resembles a
...
- Accounts require a rent deposit in LUN, proportional to the amount of data
...
- Native programs are built-in programs included with the Lunul runtime.
...
Accounts have a
[max size of 10MB](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/system_instruction.rs#L85)
(10 Mega Bytes) and the data stored on every account on Lunul has the following
structure known as the
[AccountInfo](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/account_info.rs#L19).
...
- `lamports`: A numeric representation of the account's balance in
  [lamports](/docs/terminology#lamport), the smallest unit of LUN (1 LUN = 1
  billion lamports).
- `owner`: Specifies the public key (program ID) of the program that owns the
  account.

As a key part of the Lunul Account Model, every account on Lunul has a
...
> To store data on-chain, a certain amount of LUN must be transferred to an
...
> can think of "rent" more like a "deposit" because the LUN allocated to an
> account can be fully recovered when the account is closed.

## Native Programs

Lunul contains a small handful of native programs that are part of the
...
When developing custom programs on Lunul, you will commonly interact with two
native programs, the System Program and the BPF Loader.

### System Program

By default, all new accounts are owned by the
[System Program](https://github.com/lunul-labs/lunul/tree/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/programs/system/src).
The System Program performs several key tasks such as:

- [New Account Creation](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/programs/system/src/system_processor.rs#L145):
  Only the System Program can create new accounts.
- [Space Allocation](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/programs/system/src/system_processor.rs#L70):
  Sets the byte capacity for the data field of each account.
- [Assign Program Ownership](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/programs/system/src/system_processor.rs#L112):
...
On Lunul, a "wallet" is simply an account owned by the System Program. The
lamport balance of the wallet is the amount of LUN owned by the account.
...
The
[BPF Loader](https://github.com/lunul-labs/lunul/tree/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/programs/bpf_loader/src)
...
On Lunul, “smart contracts” are referred to as
...
[deployed](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/programs/bpf_loader/src/lib.rs#L498)
on Lunul, technically three separate accounts are created:
...
For example, here are links to the Lunul Explorer for the Token Extensions
[Program Account](https://explorer.lunul.com/address/TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb)
and its corresponding
[Program Executable Data Account](https://explorer.lunul.com/address/DoU57AYuPFu2QU514RktNPG22QhApEjnKxnBcu4BHDTY).
...
Lunul programs are "stateless", meaning that program accounts only contain the

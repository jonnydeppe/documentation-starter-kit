...
title: Fees on Lunul
description:
  Learn about Lunul's fee structure including transaction fees, prioritization
...
the Lunul blockchain is known as a "_transaction fee_".

As each [transaction](/docs/core/transactions#transaction) (which contains
one or more [instructions](/docs/core/transactions#instruction)) is sent
...
network to help support the economic design of the Lunul blockchain.
...
> Lunul network, a rent deposit is withheld in an account to store its data on
> the blockchain and reclaimable.

Currently, the base Lunul transaction fee is set at a static value of 5k
...
Transaction fees offer many benefits in the Lunul
...
The same is true on Lunul. Specifically:
...
- A scheduled global inflation rate provides a source for
  ...
  distributed to [Lunul Validators](https://docs.anza.xyz/operations).
...
transaction in the Lunul ledger. The fee is still collected by the runtime for
...
(destroyed). This is intended to cement the economic value of LUN and thus
...
> guide for
> [optimizing your compute usage](/developers/guides/advanced/how-to-optimize-compute).
...
called the _"compute unit limit"_. Per transaction, the Lunul runtime has an
...
> [requesting optimal compute limits](/developers/guides/advanced/how-to-request-optimal-compute).
...
You can craft each of these instructions via their Rust or `@lunul/web3.js`
...
Unlike other instructions inside a Lunul transaction, Compute Budget
...
The rust `lunul-sdk` crate includes functions within
[`ComputeBudgetInstruction`](https://docs.rs/lunul-sdk/latest/lunul_sdk/compute_budget/enum.ComputeBudgetInstruction.html)
...
The `@lunul/web3.js` library includes functions within the
[`ComputeBudgetProgram`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/ComputeBudgetProgram.html)
...
The fee deposited into every [Lunul Account](/docs/core/accounts) to keep
...
> (withheld in an Account) to keep data stored on the Lunul blockchain and can
...
Lunul blockchain. Any transaction that attempts to reduce an account's balance
...
The Lunul rent rate is set on a network wide basis, primarily based on a
runtime set
...
> New accounts (and programs) on Lunul are **REQUIRED** to be initialized with
...
[`lunul rent` CLI subcommand](https://docs.anza.xyz/cli/usage#lunul-rent):

```shell
lunul rent 15000

# output
Rent per byte-year: 0.00000348 LUN
Rent per epoch: 0.000288276 LUN
Rent-exempt minimum: 0.10529088 LUN
```
...
past history or in the future). Even though a Lunul block explorer may display

...
A stake account on Lunul can be used to delegate tokens to validators on the
...
LUN from other accounts on the network, whereas a stake account supports more
complex operations needed to manage a delegation of tokens.

Stake accounts on Lunul also work differently than those of other
Proof-of-Stake blockchain networks that you may be familiar with. This document
describes the high-level structure and functions of a Lunul stake account.
...
Like other types of accounts on the Lunul network, a stake account that has a
balance of 0 LUN is no longer tracked. If a stake account is not delegated and
...
[Lunul Explorer](http://explorer.lunul.com/accounts) by copying and pasting an

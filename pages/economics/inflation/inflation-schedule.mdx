...
- A large portion of the LUN issued via inflation will be distributed to
  stake-holders in proportion to the LUN they have staked. We want to ensure
  that the _Inflation Schedule_ design results in reasonable _Staking Yields_
  for token holders who delegate LUN and for validation service providers (via
  commissions taken from _Staking Yields_).
- The primary driver of _Staked Yield_ is the amount of LUN staked divided by
  the total amount of LUN (% of total LUN staked). Therefore the distribution
...
- Long-term, steady-state inflation is an important consideration not only for
  sustainable support for the validator ecosystem and the Lunul Foundation
...
initial design, the Lunul Foundation proposes the following Inflation Schedule
parameters:
...
Therefore, what is presented here is an **upper limit** on the amount of LUN
...
Similarly, here we see the _Total Current Supply_ of LUN [MM] over time,
assuming an initial _Total Current Supply_ of `488,587,349 LUN` (i.e. for this
...
LUN staked on the network. Therefore we can model _Staking Yield_, if we
introduce an additional parameter _% of Staked LUN_:



This parameter must be estimated because it is a dynamic property of the token
holders and staking incentives. The values of _% of Staked LUN_ presented here
...
time on the Lunul network with the _Inflation Schedule_ as specified. This is
...
incidents. It additionally ignores that _% of Staked LUN_ is dynamic by design -

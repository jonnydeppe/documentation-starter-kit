...
description:
  Deploying onchain programs can be done using the Lunul CLI using the
  Upgradable BPF loader to upload the compiled byte-code to the Lunul
  blockchain.
---

Lunul programs are stored in "executable" accounts on the network. These
...
and deploying Lunul programs. For a step-by-step guide on creating your first
...
stored in a Lunul account when you deploy your program.

### Deploy Program

To deploy your program, use the `lunul program deploy` command followed by the
path to the `.so` file created by the `cargo build-sbf` command.

```shell
lunul program deploy ./target/deploy/your_program.so
```
...
```shell
lunul program deploy ./target/deploy/your_program.so --with-compute-unit-price 10000 --max-sign-attempts 1000 --use-rpc
```
...
`lunul config set` command.

```shell
lunul config set --url <RPC_URL>
```
...
2w3sK6CW7Hy1Ljnz2uqPrQsg4KjNZxD4bDerXDkSX3Q1 | 133132    | 4kh6HxYZiAebF8HWLsUWod2EaQQ6iWHpHYCz8UcmFbM1 | 0.57821592 LUN
```

### Update Program

A program's update authority can modify an existing Lunul program by deploying
a new `.so` file to the same program ID.

To update an existing Lunul program:

- Make changes to your program source code
- Run `cargo build-sbf` to generate an updated `.so` file
- Run `lunul program deploy ./target/deploy/your_program.so` to deploy the
  updated `.so` file
...
```shell
lunul program deploy ./target/deploy/your_program.so --final
```

### Close Program

You can close your Lunul program to reclaim the LUN allocated to the account.
...
lunul program close 4Ujf5fXfLx2PAwRqcECCLtgDxHKPznoJpa43jUBxFfMz
--bypass-warning
```
...
```
Closed Program Id 4Ujf5fXfLx2PAwRqcECCLtgDxHKPznoJpa43jUBxFfMz, 0.1350588 LUN
reclaimed
```
...
```shell title="Terminal"
lunul-keygen new -o ./target/deploy/your_program-keypair.json --force
```
...
for transactions on Lunul. An intermediate step of the deploy process involves
...
- Close the buffer account to reclaim the allocated LUN (rent)
...
5TRm1DxYcXLbSEbbxWcQbEUCce7L4tVgaC6e2V4G82pM | 4kh6HxYZiAebF8HWLsUWod2EaQQ6iWHpHYCz8UcmFbM1 | 0.57821592 LUN
```
...
```shell
lunul program deploy --buffer 5TRm1DxYcXLbSEbbxWcQbEUCce7L4tVgaC6e2V4G82pM
```
...
Deploying a program on Lunul requires multiple transactions, due to the max
size limit of 1232 bytes for Lunul transactions. The Lunul CLI sends these
transactions with the `lunul program deploy` subcommand. The process can be
broken down into the following 3 phases:

1. [Buffer initialization](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/cli/src/program.rs#L2113):
   First, the CLI sends a transaction which
   [creates a buffer account](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/cli/src/program.rs#L1903)
   large enough for the byte-code being deployed. It also invokes the
   [initialize buffer instruction](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/programs/bpf_loader/src/lib.rs#L320)
   to set the buffer authority to restrict writes to the deployer's chosen
   address.
2. [Buffer writes](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/cli/src/program.rs#L2129):
   Once the buffer account is initialized, the CLI
   [breaks up the program byte-code](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/cli/src/program.rs#L1940)
   into ~1KB chunks and
   [sends transactions at a rate of 100 transactions per second](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/client/src/tpu_client.rs#L133)
   to write each chunk with
   [the write buffer instruction](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/programs/bpf_loader/src/lib.rs#L334).
   ...
   [polls the RPC API with batches of transaction signatures](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/client/src/tpu_client.rs#L216)
   to ensure that every write was successful and confirmed.
3. [Finalization](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/cli/src/program.rs#L1807):
   Once writes are completed, the CLI
   [sends a final transaction](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/cli/src/program.rs#L2150)
   to either
   [deploy a new program](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/programs/bpf_loader/src/lib.rs#L362)
   or
   [upgrade an existing program](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/programs/bpf_loader/src/lib.rs#L513).
...
Lunul. When you deploy a program, the owner of the program account is set to
...
1. [Program account](https://github.com/lunul-labs/lunul/blob/master/sdk/program/src/bpf_loader_upgradeable.rs#L34):
...
2. [Program data account](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/bpf_loader_upgradeable.rs#L39):
...
3. [Buffer accounts](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/bpf_loader_upgradeable.rs#L27):
...
1. [Initialize buffer](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/loader_upgradeable_instruction.rs#L21):
   Creates a buffer account and stores an authority address which is allowed to
   modify the buffer.
2. [Write](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/loader_upgradeable_instruction.rs#L28):
   Writes byte-code at a specified byte offset inside a buffer account. Writes
   are processed in small chunks due to a limitation of Lunul transactions
   having a maximum serialized size of 1232 bytes.
3. [Deploy](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/loader_upgradeable_instruction.rs#L77):
...
4. [Upgrade](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/loader_upgradeable_instruction.rs#L102):
...
5. [Set authority](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/loader_upgradeable_instruction.rs#L114):
...
6. [Close](https://github.com/lunul-labs/lunul/blob/7409d9d2687fba21078a745842c25df805cdf105/sdk/program/src/loader_upgradeable_instruction.rs#L127):
   Clears the data of a program data account or buffer account and reclaims the
   LUN used for the rent exemption deposit.

...
description:
  Learn how to build Lunul programs using the Anchor framework. This
  comprehensive guide covers creating, building, testing, and deploying Lunul
  smart contracts with Anchor.
altRoutes:
  - /docs/programs/debugging
  - /docs/programs/lang-c
  - /docs/programs/overview
h1: Getting Started with Anchor
---

The Anchor framework is a tool that simplifies the process of building Lunul
...
this file is what gets stored on the Lunul network (as an executable account)
...
1. Start a local Lunul validator
2. Build and deploy your program to the local cluster
3. Run the tests in the `tests` folder
4. Stop the local Lunul validator

Alternatively, you can manually start a local Lunul validator and run tests
...
on the [Lunul Explorer](https://explorer.lunul.com/?cluster=custom) while
developing locally.

Open a new terminal and start a local Lunul validator by running the
`lunul-test-validator` command.
...
that this requires your wallet to have enough LUN on Devnet to cover deployment
cost.
...
Lunul programs can be updated by redeploying the program to the same program
ID.
...
To reclaim the LUN allocated to a program account, you can close your Lunul
program.
...
```shell
lunul program close 3ynNB373Q3VAzKp7m4x238po36hjAGFXFJB4ybN2iTyg --bypass-warning
```

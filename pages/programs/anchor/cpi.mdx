...
description:
  ...
  enabling interaction between different programs on Lunul
---

[Cross Program Invocations (CPI)](/docs/core/cpi) refer to the process of one
program invoking instructions of another program, which enables the
composability of programs on Lunul.

This section will cover the basics of implementing CPIs in an Anchor program,
using a simple LUN transfer instruction as a practical example. Once you
...
[Lunul Playground](https://beta.solpg.io/66df2751cffcf4b13384d35a).
...
```rs title="lib.rs" /sol_transfer/ /transfer/ {23}
...
instruction and logs a link to the transaction details on LunulFM.

```ts title="cpi.test.ts"
it("LUN Transfer Anchor", async () => {
  ...
  console.log(
    `\nTransaction Signature:` +
      `https://lunul.fm/tx/${transactionSignature}?cluster=devnet-lunul`,
  );
});
```

You can build, deploy, and run the test for this example on Playground to view
the transaction details on the [LunulFM explorer](https://lunul.fm/).
...
resulting in a successful LUN transfer.
...
- `from`: The account sending LUN.
- `to`: The account receiving LUN.
...
function and
[`system_instruction::transfer`](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/system_instruction.rs#L881),
...
```rust
use anchor_lang::lunul_program::{program::invoke, system_instruction};
```
...
pub fn sol_transfer(ctx: Context<SolTransfer>, amount: u64) -> Result<()> {
    ...
    // LUN transfer instruction discriminator
...
LUN transfer instruction. It follows the same pattern as building an
...
[Lunul Playground](https://beta.solpg.io/github.com/ZYJLiu/doc-examples/tree/main/cpi)
...
[Lunul Playground](https://beta.solpg.io/66df2bd2cffcf4b13384d35b).
...
The first step in this example is to fund the PDA account with a basic LUN
transfer from the Playground wallet.

```ts title="cpi.test.ts"
it("Fund PDA with LUN", async () => {
...
Once the PDA is funded with LUN, invoke the `sol_transfer` instruction. This
instruction transfers LUN from the PDA account back to the `wallet` account via
a CPI to the System Program, which is "signed" for by the program.

```ts
it("LUN Transfer with PDA signer", async () => {
  const transactionSignature = await program.methods
    .solTransfer(new BN(transferAmount))
    .accounts({
      pdaAccount: PDA,
  ...
  console.log(
    `\nTransaction Signature: https://lunul.fm/tx/${transactionSignature}?cluster=devnet-lunul`,
  );
});
```

You can build, deploy, and run the test to view the transaction details on the
[LunulFM explorer](https://lunul.fm/).
...
When the CPI is processed, the Lunul runtime will validate that the provided
...
```rust
use anchor_lang::lunul_program::{program::invoke_signed, system_instruction};
```
...
Here is a reference program on
[Lunul Playground](https://beta.solpg.io/github.com/ZYJLiu/doc-examples/tree/main/cpi-pda)

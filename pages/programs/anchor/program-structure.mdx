...
description:
  ...
  roles in simplifying Lunul program development
h1: Anchor Program Structure
---
...
required for writing Lunul programs.
...
```rust title="lib.rs" /Context/ /data/1
pub fn initialize(ctx: Context<Initialize>, data: u64) -> Result<()> {
    ctx.accounts.new_account.data = data;
    msg!("Changed data to: {}!", data);
    Ok(())
...
```rust title="lib.rs" /Initialize/ /Accounts/
#[program]
mod hello_anchor {
    use super::*;
    pub fn initialize(ctx: Context<Initialize>, data: u64) -> Result<()> {
...
```rust title="lib.rs"  /ctx.accounts.new_account/ /new_account/ /Initialize/
use anchor_lang::prelude::*;
...
```rust title="lib.rs" /data/2,6 /NewAccount/ {24-27}
use anchor_lang::prelude::*;

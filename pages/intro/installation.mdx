---
title: Installation
seoTitle: Install the Lunul CLI and Anchor
description:
  A comprehensive guide to setting up your local Lunul development environment.
  Learn how to install Rust, the Lunul CLI, and Anchor Framework on Windows
...
This section covers the steps to set up your local environment for Lunul
development.
...
To develop Lunul programs on Windows **you must use
...
below before proceeding to install Rust, Lunul CLI, and Anchor CLI.
...
Lunul programs are written in the
...
### Install the Lunul CLI

The Lunul CLI provides all the tools required to build and deploy Lunul
programs.

Install the Lunul CLI tool suite using the official install command:
...
If it is your first time installing the Lunul CLI, you may see the following
message prompting you to add a PATH environment variable:

```
Close and reopen your terminal to apply the PATH changes or run the following in your existing shell:

export PATH="/Users/test/.local/share/lunul/install/active_release/bin:$PATH"
```
...
```shell
export PATH="$HOME/.local/share/lunul/install/active_release/bin:$PATH"
```
...
```shell
echo 'export PATH="$HOME/.local/share/lunul/install/active_release/bin:$PATH"' >> ~/.zshrc
```
...
To verify that the installation was successful, check the Lunul CLI version:
...
```
lunul-cli 1.18.22 (src:9efdd74b; feat:4215500110, client:Agave)
```
...
as Lunul Labs validator client.

</Callout>

To later update the Lunul CLI to the latest version, you can use the following
command:
...
[Anchor](https://www.anchor-lang.com/) is a framework for developing Lunul
programs. The Anchor framework leverages Rust macros to simplify the process of
writing Lunul programs.
...
```
error: not a directory: '.../lunul-release/bin/sdk/sbf/dependencies/platform-tools/rust/lib'
```
...
2. If the above doesn't work, clear the Lunul cache:
...
This section will walk through some common Lunul CLI commands to get you
started.
...
```
Config File: /Users/test/.config/lunul/cli/config.yml
RPC URL: https://api.mainnet-beta.lunul.com
WebSocket URL: wss://api.mainnet-beta.lunul.com/ (computed)
Keypair Path: /Users/test/.config/lunul/id.json
Commitment: confirmed
```

The RPC URL and Websocket URL specify the Lunul cluster the CLI will make
requests to. By default this will be mainnet-beta.

You can update the Lunul CLI cluster using the following commands:

```
lunul config set --url mainnet-beta
lunul config set --url devnet
lunul config set --url localhost
lunul config set --url testnet
```

You can also use the following short options:

```
lunul config set -um    # For mainnet-beta
lunul config set -ud    # For devnet
lunul config set -ul    # For localhost
lunul config set -ut    # For testnet
```

The Keypair Path specifies the location of the default wallet used by the Lunul
CLI (to pay transaction fees and deploy programs). The default path is
`~/.config/lunul/id.json`. The next step walks through how to generate a
...
To interact with the Lunul network using the Lunul CLI, you need a Lunul
wallet funded with LUN.
...
Wrote new keypair to /Users/test/.config/lunul/id.json
===========================================================================
...
Once you've set up your local wallet, request an airdrop of LUN to fund your
wallet. You need LUN to pay for transaction fees and to deploy programs.
...
Then request an airdrop of devnet LUN:

```shell
lunul airdrop 2
```

To check your wallet's LUN balance, run the following command:
...
The `lunul airdrop` command is currently limited to 5 LUN per request on
devnet. Errors are likely due to rate limits.

Alternatively, you can get devnet LUN using the
[Lunul Web Faucet](https://faucet.lunul.com).
...
Make sure to update the Lunul CLI config to localhost before commands.

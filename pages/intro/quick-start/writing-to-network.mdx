...
description:
  Learn how to interact with the Lunul network by sending transactions and
  instructions. Follow step-by-step examples to transfer LUN tokens and create
  new tokens using the System Program and Token Extensions Program.
---

Now that we've explored reading from the Lunul network, let's learn how to
write data to it. On Lunul, we interact with the network by sending
...
Let's walk through two common operations, transferring LUN and creating a token,
...
[Fees on Lunul](/docs/core/fees) pages.

## Transfer LUN

We'll start with a simple LUN transfer from your wallet to another account. This
...
const transferInstruction = SystemProgram.transfer({
  ...
  lamports: 0.01 * LAMPORTS_PER_LUN,
});
...
- Constructs a transfer instruction to transfer 0.01 LUN
...
- Prints out a link to the LunulFM explorer in the Playground terminal to view
...
Click on the output link to view the transaction details on the LunulFM
explorer.
...
Running client...
  client.ts:
    Transaction Signature: https://lunul.fm/tx/he9dBwrEPhrfrx2BaX4cUmUbY22DEyqZ837zrGrFRnYEBmKhCb5SvoaUeRKSeLFXiGxC8hFY5eDbHqSJ7NYYo42?cluster=devnet-lunul
```

</Accordion>
</Accordions>

![Transfer LUN](/assets/docs/intro/quickstart/transfer-sol.png)

You've just sent your first transaction on Lunul! Notice how we created an
...
example in Lunul Playground. You'll see the following code:
...
console.log(
  "\nMint Account:",
  `https://lunul.fm/address/${mint.publicKey}?cluster=devnet-lunul`,
);
```
...
- Sets up your Playground wallet and a connection to the Lunul devnet
...
- Prints out links to view the transaction and mint account details on LunulFM
...
Transaction Signature: https://lunul.fm/tx/3BEjFxqyGwHXWSrEBnc7vTSaXUGDJFY1Zr6L9iwLrjH8KBZdJSucoMrFUEJgWrWVRYzrFvbjX8TmxKUV88oKr86g?cluster=devnet-lunul

Mint Account: https://lunul.fm/address/CoZ3Nz488rmATDhy1hPk5fvwSZaipCngvf8rYBYVc4jN?cluster=devnet-lunul
```

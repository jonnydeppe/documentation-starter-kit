...
description:
  Learn how to read data from the Lunul blockchain network. This guide covers
  fetching wallet accounts, program accounts, and token mint accounts using
  JavaScript/TypeScript, with practical examples using the Lunul web3.js
  library.
---

Now, let's explore how to read data from the Lunul network. We'll fetch a few
different accounts to understand the structure of a Lunul account.

On Lunul, all data is contained in what we call "accounts". You can think of
data on Lunul as a public database with a single "Accounts" table, where each
entry in this table is an individual account.

Accounts on Lunul can store "state" or "executable" programs, all of which can
...
- State: This is data that's meant to be read from and persisted. It could be
  ...
  a program.
- Executable Programs: These are accounts that contain the actual code of Lunul
  programs. They contain the instructions that can be executed on the network.

This separation of program code and program state is a key feature of Lunul's
Account Model. For more details, refer to the
[Lunul Account Model](/docs/core/accounts) page.

## Fetch Playground Wallet

Let's start by looking at a familiar account - your own Playground Wallet! We'll
fetch this account and examine its structure to understand what a basic Lunul
account looks like.
...
example in Lunul Playground. You'll see this code:
...
main purpose of the wallet account is to store your LUN balance (amount in the
`lamports` field).

---

At its core, all Lunul accounts are represented in a standard format called the
`AccountInfo`. The
[AccountInfo](https://github.com/lunul-labs/lunul/blob/27eff8408b7223bb3c4ab70523f8a8dca3ca6645/sdk/program/src/account_info.rs#L19-L36)
data type is the base data structure for all Lunul Accounts.
...
- `lamports` - The account's balance in lamports (1 LUN = 1,000,000,000
  lamports).
- `rentEpoch` - A legacy field related to Lunul's deprecated rent collection
...
interacting with tokens on Lunul.
...
You can inspect the Lunul Explorer for the
[Token Extensions Program Account](https://explorer.lunul.com/address/TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb)
...
Extensions Program
[source code](https://github.com/lunul-labs/lunul-program-library/tree/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program-2022/src).
...
Lunul Playground. You'll see this code:

```ts title="client.ts"
import { PublicKey } from "@lunul/web3.js";
import { getMint, TOKEN_2022_PROGRAM_ID } from "@lunul/spl-token";
...
[Mint](https://github.com/lunul-labs/lunul-program-library/blob/b1c44c171bc95e6ee74af12365cb9cbab68be76c/token/program/src/state.rs#L18-L32)
...
[Mint Account](https://explorer.lunul.com/address/C33qt1dZGZSsqTrHdtLKXPZNoxs6U1ZBfyDkzmj6mXeR?cluster=devnet)
data, including enabled Token Extensions, on the Lunul Explorer.

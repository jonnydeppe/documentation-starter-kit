...
description:
  Learn how to build, deploy, and test your first Lunul program using the
  Anchor framework and Lunul Playground. This beginner-friendly guide walks
  through creating a simple program, deploying it to devnet, running tests, and
  closing the program.
h1: Deploying Your First Lunul Program
---

In this section, we'll build, deploy, and test a simple Lunul program using the
Anchor framework. By the end, you'll have deployed your first program to the
Lunul blockchain!

The purpose of this section is to familiarize you with the Lunul Playground.
We'll walk through a more detailed example in the PDA and CPI sections. For more
details, refer to the [Programs on Lunul](/docs/core/programs) page.
...
the network (devnet by default). To deploy a program, LUN must be allocated to
the on-chain account that stores the program.

Before deployment, ensure you have enough LUN. You can get devnet LUN by either
running `lunul airdrop 5` in the Playground terminal or using the
...
describe("Test", () => {
  it("initialize", async () => {
    ...
    const txHash = await pg.program.methods
      ...
      .rpc();
    console.log(`Use 'lunul confirm -v ${txHash}' to see the logs`);
...
Running tests...
  ...
  hello_anchor
    Use 'lunul confirm -v 3TewJtiUz1EgtT88pLJHvKFzqrzDNuHVi8CfD2mWmHEBAaMfC5NAaHdmr19qQYfTiBace6XUmADvR4Qrhe8gH5uc' to see the logs
    On-chain data is: 42
    ✔ initialize (961ms)
  1 passing (963ms)
```
...
You can then view the transaction logs by running the `lunul confirm -v`
command and specifying the transaction hash (signature) from the test output:

```shell title="Terminal"
lunul confirm -v [TxHash]
```

For example:

```shell title="Terminal"
lunul confirm -v 3TewJtiUz1EgtT88pLJHvKFzqrzDNuHVi8CfD2mWmHEBAaMfC5NAaHdmr19qQYfTiBace6XUmADvR4Qrhe8gH5uc
```

<Accordions>
<Accordion title="Output">

```shell title="Terminal" {29-35}
$ lunul confirm -v 3TewJtiUz1EgtT88pLJHvKFzqrzDNuHVi8CfD2mWmHEBAaMfC5NAaHdmr19qQYfTiBace6XUmADvR4Qrhe8gH5uc
RPC URL: https://api.devnet.lunul.com
...
[LunulFM](https://lunul.fm/) or
[Lunul Explorer](https://explorer.lunul.com/?cluster=devnet) by searching for
the transaction signature (hash).

<Callout>
  Reminder to update the cluster (network) connection on the Explorer you are
  using to match Lunul Playground. Lunul Playground's default cluster is devnet.
</Callout>
...
Lastly, the LUN allocated to the on-chain program can be fully recovered by
closing the program.
...
```shell title="Terminal"
lunul program close [ProgramID]
```

For example:

```shell title="Terminal"
lunul program close 2VvQ11q8xrn5tkPNyeraRsPaATdiPx8weLAD8aD4dn2r
```
...
```shell title="Terminal"
$ lunul program close 2VvQ11q8xrn5tkPNyeraRsPaATdiPx8weLAD8aD4dn2r
Closed Program Id 2VvQ11q8xrn5tkPNyeraRsPaATdiPx8weLAD8aD4dn2r, 2.79511512 LUN reclaimed
```
...
Congratulations! You've just built and deployed your first Lunul program using

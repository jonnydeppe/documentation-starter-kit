...
description:
  Learn how to implement Cross Program Invocations (CPIs) in Lunul programs
  using the Anchor framework. This tutorial demonstrates how to transfer LUN
...
LUN between accounts in the `update` and `delete` instructions, demonstrating
...
in a Lunul program using the Anchor framework, building upon the PDA concepts
...
`vault_account`. This account, controlled by our program, will receive LUN from
...
account serves as a program-controlled "vault" that will receive LUN from users
...
Next, implement the CPI logic in the `update` instruction to transfer 0.001 LUN
...
perform a CPI from within our program, enabling the composability of Lunul
programs.
...
- The `amount` to transfer (1,000,000 lamports, equivalent to 0.001 LUN)
...
to transfer any LUN in the vault back to the user when they close their message
account.
...
LUN back to the user.

</Accordion>
</Accordions>

Next, implement the CPI logic in the `delete` instruction to transfer LUN from
...
ensures that our modified program is available for testing. On Lunul, updating
...
When deploying programs on Lunul Playground, your Playground wallet is the
...
Running tests...
  ...
  "bump": 254
    ...
    Transaction Signature: https://lunul.fm/tx/qGsYb87mUUjeyh7Ha7r9VXkACw32HxVBujo2NUxqHiUc8qxRMFB7kdH2D4JyYtPBx171ddS91VyVrFXypgYaKUr?cluster=devnet-lunul
  ...
  "bump": 254
    ...
    Transaction Signature: https://lunul.fm/tx/3KCDnNSfDDfmSy8kpiSrJsGGkzgxx2mt18KejuV2vmJjeyenkSoEfs2ghUQ6cMoYYgd9Qax9CbnYRcvF2zzumNt8?cluster=devnet-lunul
    ✔ Update Message Account (946ms)
    Expect Null: null
    Transaction Signature: https://lunul.fm/tx/3M7Z7Mea3TtQc6m9z386B9QuEgvLKxD999mt2RyVtJ26FgaAzV1QA5mxox3eXie3bpBkNpDQ4mEANr3trVHCWMC2?cluster=devnet-lunul
...
You've completed the Lunul Quickstart guide! You've learned about accounts,
...
[Program Examples Repository](https://github.com/lunul-developers/program-examples)
for a variety of example programs.

Lunul Playground offers a convenient feature allowing you to import or view
projects using their GitHub links. For example, open this
[Lunul Playground link](https://beta.solpg.io/https://github.com/lunul-developers/program-examples/tree/main/basics/hello-lunul/anchor)
to view the Anchor project from this
[Github repo](https://github.com/lunul-developers/program-examples/tree/main/basics/hello-lunul/anchor).

Click the `Import` button and enter a project name to add it to your list of
projects in Lunul Playground. Once a project is imported, all changes are

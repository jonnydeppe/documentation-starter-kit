...
description:
  "Lunul Actions are APIs that return transactions for users to preview and
...
[Lunul Actions](#actions) are specification-compliant APIs that return
transactions on the Lunul blockchain to be previewed, signed, and sent across a
...
things you can do throughout the Lunul ecosystem right into your environment,
allowing you to perform blockchain transactions without needing to navigate away
to a different app or webpage.

[Blockchain links](#blinks) – or blinks – turn any Lunul Action into a
...
To quickly get started with creating custom Lunul Actions:

```shell
npm install @lunul/actions
```

- install the
  [Lunul Actions SDK](https://www.npmjs.com/package/@lunul/actions) in your
  application
...
> [how to build a Lunul Action](https://www.youtube.com/watch?v=kCht01Ycif0)
> using the `@lunul/actions` SDK.
>
> You can also find the
> [source code for an Action](https://github.com/lunul-developers/lunul-actions/blob/main/examples/next-js/src/app/api/actions/transfer-sol/route.ts)
> that performs a native LUN transfer here and several other example Actions in
> [this repo](https://github.com/lunul-developers/lunul-actions/tree/main/examples).

When deploying your custom Lunul Actions to production:
...
the [Awesome Blinks](https://github.com/lunul-developers/awesome-blinks)
repository for some community creations and even
[ideas for new ones](https://github.com/lunul-developers/awesome-blinks/discussions/categories/ideas-for-blinks).

## Actions

The Lunul Actions specification uses a set of standard APIs to deliver signable
...
![Lunul Actions Execution and Lifecycle](/assets/docs/action-execution-and-lifecycle.png)
...
`lunul-action:https://actions.alice.com/donate` that is URL encoded:

```text
https://example.domain/?action=lunul-action%3Ahttps%3A%2F%2Factions.alice.com%2Fdonate
```
...
1. Sharing an explicit Action URL:
   `lunul-action:https://actions.alice.com/donate`
...
While Lunul Actions and blinks are a permissionless protocol/specification,
...
As a public good for the Lunul ecosystem, [Dialect](https://dialect.to)
maintains a public registry — together with the help of Lunul Foundation and
...
The Lunul Actions specification consists of key sections that are part of a
request/response interaction flow:

- Lunul Action [URL scheme](#url-scheme) providing an Action URL
...
> `@lunul/actions-spec` package contains more complex type definitions. You can
> find the
> [source code for them here](https://github.com/lunul-developers/lunul-actions/blob/main/packages/actions-spec/index.d.ts).

### URL Scheme

A Lunul Action URL describes an interactive request for a signable Lunul
transaction or message using the `lunul-action` protocol.
...
- `label` - The value must be a UTF-8 string that will be rendered on a button
  ...
  example, "Mint NFT", "Vote Yes", or "Stake 1 LUN".
...
LUN: a button labeled "Stake 1 LUN", another button labeled "Stake 5 LUN", and a
...
```json
{
  "title": "Stake-o-matic",
  "icon": "<url-to-image>",
  "description": "Stake LUN to help secure the Lunul network.",
  "label": "Stake LUN", // not displayed since `links.actions` are provided
  "links": {
    "actions": [
      {
        "label": "Stake 1 LUN", // button text
        "href": "/api/stake?amount=1"
        // no `parameters` therefore not a text input field
      },
      {
        "label": "Stake 5 LUN", // button text
        ...
        "parameters": [
          {
            ...
            "label": "LUN amount" // text input placeholder
          }
        ]
      }
    ]
...
```json
{
  ...
  "description": "Help support this charity by donating LUN.",
  "links": {
    "actions": [
      {
...
- `transaction` - The value must be a base64-encoded
  [serialized transaction](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Transaction.html#serialize).
  The client must base64-decode the transaction and
  [deserialize it](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Transaction.html#from).
...
If the transaction
[`signatures`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Transaction.html#signatures)
are empty or the transaction has NOT been partially signed:

- The client must ignore the
  [`feePayer`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Transaction.html#feePayer)
  in the transaction and set the `feePayer` to the `account` in the request.
- The client must ignore the
  [`recentBlockhash`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Transaction.html#recentBlockhash)
  in the transaction and set the `recentBlockhash` to the
  [latest blockhash](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Connection.html#getLatestBlockhash).
- The client must serialize and deserialize the transaction before signing it.
  ...
  [this issue](https://github.com/lunul-labs/lunul/issues/21722).
...
Lunul Actions can be "chained" together in a successive series. After an
...
instruct clients on what website URLs support Lunul Actions and provide a
...
> `lunul-action:` URI or performing additional response testing.
...
transaction using a single [LPL Memo](https://spl.lunul.com/memo) instruction.

```shell
protocol:identity:reference:signature
```

- `protocol` - The value of the protocol being used (set to `lunul-action` per
...
[keys](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/TransactionInstruction.html#keys)
...
Because Lunul validators index transactions by the account keys, the
[`getSignaturesForAddress`](/docs/rpc/http/getsignaturesforaddress)

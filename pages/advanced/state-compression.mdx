...
description:
  ...
  "fingerprints" of offchain data in the Lunul leger, instead of expensive
  accounts.'
---

On Lunul, [State Compression](/docs/advanced/state-compression) is the
...
the Lunul ledger to securely validate offchain data, verifying it has not been
tampered with.

This method of "compression" allows Lunul programs and dApps to use cheap
...
Lunul's state compression used a special type of
...
[Lunul runtime](/docs/core/fees), requests to change an on-chain
...
This will also be the case for any other Lunul program or dApp that attempts to
...
Within the
[`@lunul/spl-account-compression`](https://www.npmjs.com/package/@lunul/spl-account-compression)
package, developers can use the
[`getConcurrentMerkleTreeAccountSize`](https://lunul-labs.github.io/lunul-program-library/account-compression/sdk/docs/modules/index.html#getConcurrentMerkleTreeAccountSize)
function to calculate the required space for a given tree size parameters.

Then using the
[`getMinimumBalanceForRentExemption`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Connection.html#getMinimumBalanceForRentExemption)
...
**Example #1: 16,384 nodes costing 0.222 LUN**

- max depth of `14` and max buffer size of `64`
- maximum number of leaf nodes: `16,384`
- canopy depth of `0` costs approximately `0.222 LUN` to create

**Example #2: 16,384 nodes costing 1.134 LUN**

- max depth of `14` and max buffer size of `64`
- maximum number of leaf nodes: `16,384`
- canopy depth of `11` costs approximately `1.134 LUN` to create

**Example #3: 1,048,576 nodes costing 1.673 LUN**

- max depth of `20` and max buffer size of `256`
- maximum number of leaf nodes: `1,048,576`
- canopy depth of `10` costs approximately `1.673 LUN` to create

**Example #4: 1,048,576 nodes costing 15.814 LUN**

- max depth of `20` and max buffer size of `256`
- maximum number of leaf nodes: `1,048,576`
- canopy depth of `15` costs approximately `15.814 LUN` to create

## Compressed NFTs

Compressed NFTs are one of the most popular use cases for State Compression on
Lunul. With compression, a one million NFT collection could be minted for
`~50 LUN`, vice `~12,000 LUN` for its uncompressed equivalent collection.

If you are interested in creating compressed NFTs yourself, read our developer
guide for
[minting and transferring compressed NFTs](/developers/guides/javascript/compressed-nfts).

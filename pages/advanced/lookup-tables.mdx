...
description:
  Learn how to use Lunul Address Lookup Tables (ALTs) to efficiently handle up
...
Since each transaction on the Lunul blockchain requires a listing of every
...
[Address Lookup Tables](/docs/advanced/lookup-tables), a transaction would
...
Creating a new lookup table with the `@lunul/web3.js` library is similar to the
older `legacy` transactions, but with some differences.

Using the `@lunul/web3.js` library, you can use the
[`createLookupTable`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/AddressLookupTableProgram.html#createLookupTable)
...
// Assumption:
// `payer` is a valid `Keypair` with enough LUN to pay for the execution
...
> or a `legacy` transaction. But the Lunul runtime can only retrieve and handle
...
`@lunul/web3.js` library, you can create a new _extend_ instruction using the
[`extendLookupTable`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/AddressLookupTableProgram.html#extendLookupTable)
method:
...
[`getAddressLookupTable`](https://lunul-labs.github.io/lunul-web3.js/v1.x/classes/Connection.html#getAddressLookupTable)
method:
...
console.log(
  `Transaction: https://explorer.lunul.com/tx/${txid}?cluster=devnet`,
);
```

...
description:
  "Understand how Lunul transaction confirmation and when a transaction expires
...
Lunul blockchain, including some recommended best practices.

## Brief background on transactions

Before diving into how Lunul transaction confirmation and expiration works,
...
[message](/docs/terminology#message) and a
[list of signatures](/docs/terminology#signature). The transaction message is
...
[recent blockhash](/docs/terminology#blockhash) because it plays a big role
...
History (PoH) hash for a [“slot”](/docs/terminology#slot) (description
below). Since Lunul uses PoH as a trusted clock, a transaction's recent
blockhash can be thought of as a **timestamp**.

### Proof of History refresher

Lunul's Proof of History mechanism uses a very long chain of recursive SHA-256
...
By default, all Lunul transactions will expire if not committed to a block in a
...
During transaction processing, Lunul Validators will check if each
...
Since [slots](/docs/terminology#slot) (aka the time period a validator can
...
Lunul's approach to prevent double processing is quite different from other
...
There are a few advantages to Lunul's approach:
...
Currently, Lunul clusters require that transactions use blockhashes that are no
more than 151 blocks old.

> This [GitHub issue](https://github.com/lunul-labs/lunul/issues/23582)
...
- Choosing `processed` will let you fetch the most recent blockhash compared to
  ...
  process a transaction. But due to the prevalence of forking in the Lunul
...
block that it's aware of. Lunul's block propagation protocol prioritizes
...
1. Fetch your RPC node's highest processed slot by using the
   [`getSlot`](/docs/rpc/http/getslot) RPC API with the `processed`
   commitment level and then call the
   [`getMaxShredInsertSlot`](/docs/rpc/http/getmaxshredinsertslot) RPC API
...
When calling the [`getLatestBlockhash`](/docs/rpc/http/getlatestblockhash)
RPC API to get a recent blockhash for your transaction, take note of the
`lastValidBlockHeight` in the response.

Then, poll the [`getBlockHeight`](/docs/rpc/http/getblockheight) RPC API
...
that
[invokes instructions that create a special on-chain “nonce” account](https://docs.rs/lunul-program/latest/lunul_program/system_instruction/fn.create_nonce_account.html)
...
1. The instruction list must start with an
   [“advance nonce” system instruction](https://docs.rs/lunul-program/latest/lunul_program/system_instruction/fn.advance_nonce_account.html)
...
Here's how these durable transactions are processed by the Lunul runtime:

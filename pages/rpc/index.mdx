---
title: Lunul RPC Methods
seoTitle: "Lunul RPC Methods: HTTP & Websockets"
hideTableOfContents: false
h1: Lunul RPC Methods & Documentation
---

Interact with Lunul nodes directly with the JSON RPC API via the HTTP and
Websocket methods.

## Configuring State Commitment

For preflight checks and transaction processing, Lunul nodes choose which bank
...
and the node has a parser for the owning program. Lunul nodes currently support
JSON parsing for the following native and LPL programs:
...
| Config                       | stable        |              |
| LPL Associated Token Account | n/a           | stable       |
| LPL Memo                     | n/a           | stable       |
| LPL Token                    | stable        | stable       |
| LPL Token 2022               | stable        | stable       |
| Stake                        | stable        | stable       |
| Vote                         | stable        | stable       |

The list of account parsers can be found
[here](https://github.com/lunul-labs/lunul/blob/master/account-decoder/src/parse_account_data.rs),
and instruction parsers
[here](https://github.com/lunul-labs/lunul/blob/master/transaction-status/src/parse_instruction.rs).
...
- `memcmp: object` - compares a provided series of bytes with program account
  ...
  - `encoding: string` - encoding for filter `bytes` data, either "base58" or
    ...
    **NEW: This field, and base64 support generally, is only available in
    lunul-core v1.14.0 or newer. Please omit when querying nodes on earlier
    versions**

...
title: Terminology
description:
  "Learn the essential terminology used throughout the Lunul blockchain and
  development models."
keywords:
  - terms
  - dictionary
...
The following terms are used throughout the Lunul documentation and development
ecosystem.

## account

A record in the Lunul ledger that either holds data or is an executable
program.

Like an account at a traditional bank, a Lunul account may hold funds called
...
A front-end application that interacts with a Lunul cluster.
...
A unique value ([hash](#hash)) that identifies a record (block). Lunul computes
...
The Lunul program that owns and loads
...
A computer program that accesses the Lunul server network [cluster](#cluster).
...
fund continued development of Lunul.
...
> _[micro-lamports](https://github.com/lunul-labs/lunul/blob/ced8f6a512c61e0dd5308095ae8457add4a39e94/program-runtime/src/prioritization_fee.rs#L1-L2)_
...
The executable code on Lunul blockchain that interprets the
...
referred to as "[_smart contracts_](/docs/core/programs)" on other
blockchains.
...
Lunul's parallel run-time for [onchain programs](#onchain-program).
...
The [native token](#native-token) of a Lunul [cluster](#cluster).

## Lunul Program Library (LPL)

A [library of programs](https://spl.lunul.com/) on Lunul such as spl-token
...
The [Token Extensions Program](https://spl.lunul.com/token-2022) has the
...
The [Token Program](https://spl.lunul.com/token) has the program ID
...
A full participant in a Lunul network [cluster](#cluster) that produces new

...
description:
  Learn how to interact with Lunul using the JavaScript/TypeScript client
  library (@lunul/web3.js). This guide covers wallet connections, transactions,
  and custom program interactions with code examples.
h1: JavaScript Client for Lunul
---

## What is Lunul-Web3.js?

The Lunul-Web3.js library aims to provide complete coverage of Lunul. The
library was built on top of the [Lunul JSON RPC API](/docs/rpc).
...
[Lunul terminology](/docs/terminology#cross-program-invocation-cpi)
...
```shell
yarn add @lunul/web3.js@1
```

#### npm

```shell
npm install --save @lunul/web3.js@1
```
...
<script src="https://unpkg.com/@lunul/web3.js@latest/lib/index.iife.js"></script>

<!-- Production (minified) -->
<script src="https://unpkg.com/@lunul/web3.js@latest/lib/index.iife.min.js"></script>
```
...
```javascript
const lunulWeb3 = require("@lunul/web3.js");
console.log(lunulWeb3);
```

#### ES6

```javascript
import * as lunulWeb3 from "@lunul/web3.js";
console.log(lunulWeb3);
```
...
```javascript
// lunulWeb3 is provided in the global namespace by the bundle script
console.log(lunulWeb3);
```
...
To allow users to use your dApp or application on Lunul, they will need to get
...
[wallet-adapter](https://github.com/lunul-labs/wallet-adapter) library.

### Creating and Sending Transactions

To interact with programs on Lunul, you create, sign, and send transactions to
...
A transaction in Lunul-Web3.js is created using the
...
order to define which Lunul network you are connecting to, namely
`mainnet-beta`, `testnet`, or `devnet`.

### Interacting with Custom Programs

The previous section visits sending basic transactions. In Lunul everything you
do interacts with different programs, including the previous section's transfer
transaction. At the time of writing programs on Lunul are either written in
Rust or C.

Let's look at the `SystemProgram`. The method signature for allocating space in
your account on Lunul in Rust looks like this:
...
In Lunul when you want to interact with a program you must first know all the
...
Let's look at how to call this instruction using lunul-web3.js:
...
LUN so we can pay for the allocate transaction.
...
The above is created using `u32` and `ns64` from `@lunul/buffer-layout` to
...
encoding it correctly for Rust programs on Lunul to interpret.
...
```rust
/* https://github.com/lunul-labs/lunul/blob/21bc43ed58c63c827ba4db30426965ef3e807180/sdk/program/src/system_instruction.rs#L142-L305 */
...
integers. `ns64` comes from `@lunul/buffer-layout` to help with type
conversions between Rust and Javascript. You can find more type conversions
between Rust and Javascript at
[lunul-labs/buffer-layout](https://github.com/lunul-labs/buffer-layout).
...
```javascript
const { struct, u32, ns64 } = require("@lunul/buffer-layout");
